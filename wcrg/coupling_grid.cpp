...
double coupling_grid::get_en_grid(const particle &pp) const
{	
        Vector3d vec = pp.get_kk();
				int bix  = pp.get_bix();
				int sp   = pp.get_sp();
        int int1 = (int) rint((vec(0) + 1.0*M_PI)*grid_nx/(2.0*M_PI) - 0.5);       
        int int2 = (int) rint((vec(1) + 1.0*M_PI)*grid_ny/(2.0*M_PI) - 0.5);       
        return en_grid[int1][int2][fs_.ind(bix,sp)];
}
...
{	
  ...
  Vector3d kk = pp.get_kk();
	...
	{
		Vector3d fold_vec = bz_.mapin(kk);
   	int int1 = (int) rint((fold_vec(0) + 1.0*M_PI)*grid_nx/(2.0*M_PI) - 0.5);       
   	int int2 = (int) rint((fold_vec(1) + 1.0*M_PI)*grid_ny/(2.0*M_PI) - 0.5);       
   	return states_grid[int1][int2][bix][sp];
	}
	else
	{
		return fs_.get_state(patch,bix,sp);
